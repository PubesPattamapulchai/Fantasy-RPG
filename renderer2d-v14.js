(() => {
  ...
  function solidWallBiome(s){return ['dungeon','mine','iceCave','citadel','core','glass'].includes(s?.locationData?.biome);}
  ...
  function frame(now){
    ...
    if(!fpsLimiter(now)){requestAnimationFrame(frame);return;}
    const dt=clamp((now-last)/1000,0,.05);last=now;const s=window.EmberfallBridge?.snapshot?.();state.lastSnapshot=s||state.lastSnapshot;
