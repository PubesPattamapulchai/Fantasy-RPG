(() => {
  ...
  function startBattle(enemy){
    ...
    state.battleCombo=0;state.battleMaxCombo=0;state.battleMomentum=0;state.battleLastAction='';state.battlePerfects=0;state.battleDamageTaken=0;state.battleTurns=0;state.timingActive=false;state.battleStance='balanced';state.battleRange='mid';state.battleCompanionUsed=false;state.companionCooldown=0;state.reactionUsed=false;state.reactionReadied=false;state.parryPrimed=false;state.parryCooldown=0;state.battleRound=1;state.battlePhase=1;state.disadvantageNext=false;state.advantageNext=state.player.job==='rogue';state.player.stamina=state.player.maxStamina||100;state.player.barrier=0;state.environmentUsed=false;state.dodgePrimed=false;state.weaponTechniqueCooldown=0;state.battleSurface=null;state.battleTacticalReads=0;state.battleReadRound=0;state.battleEnvironment=BATTLE_ENVIRONMENTS[currentLocation().biome]||BATTLE_ENVIRONMENTS.grass;
