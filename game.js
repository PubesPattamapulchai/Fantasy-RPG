(() => {
  ...
  function useCompanionAssist(){
    ...
    const bond=bondMultiplier(),tactic=state.companionTactic||'assault',covenant=sigilIs('covenant'),damageMult=(tactic==='assault'?1.22:1)*(covenant?1.18:1),supportMult=(tactic==='support'?1.25:1)*(covenant?1.12:1),fast=hasTalent('kindledOath')||bondRank()==='OATHBOUND'||tactic==='support'||covenant;state.companionCooldown=fast?2:3;advanceCombo('companion',14);
    ...
    if(tactic==='guardian')gainBarrier(p.maxHp*.16,'PARTY WARD');
    spawnFx('word',`${companionAssistName().toUpperCase()} · ${partyTactic().name}`);chord([330,440,660]);updateBattleUi();return true;
  }
  ...
  function startBattle(enemy){
    ...
    state.battleCombo=0;state.battleMaxCombo=0;state.battleMomentum=0;state.battleLastAction='';state.battlePerfects=0;state.battleDamageTaken=0;state.battleTurns=0;state.timingActive=false;state.battleStance='balanced';state.battleRange='mid';state.battleCompanionUsed=false;state.companionCooldown=0;state.reactionUsed=false;state.reactionReadied=false;state.parryPrimed=false;state.parryCooldown=0;state.battleRound=1;state.battlePhase=1;state.disadvantageNext=false;state.advantageNext=state.player.job==='rogue';state.player.stamina=state.player.maxStamina||100;state.player.barrier=0;state.environmentUsed=false;state.dodgePrimed=false;state.weaponTechniqueCooldown=0;state.battleSurface=null;state.battleEnvironment=BATTLE_ENVIRONMENTS[currentLocation().biome]||BATTLE_ENVIRONMENTS.grass;
